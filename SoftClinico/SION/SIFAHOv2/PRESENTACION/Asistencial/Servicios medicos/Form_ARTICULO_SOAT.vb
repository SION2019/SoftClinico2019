Public Class Form_ARTICULO_SOAT
    Dim enlce_dta As BindingSource = New BindingSource
    Dim i, c As Integer
    Dim cups As New ARTICULO_D
    ...
    Private Sub btguardar_Click(sender As Object, e As EventArgs) Handles btguardar.Click
        ...
        Dim vArticuloSoat = buildArticuloSoat()
        If ((vArticuloSoat IsNot Nothing) AndAlso (cups.guardar(vArticuloSoat) = True)) Then
    ...
    Private Function buildArticuloSoat() As ArticuloSoat
        Dim vArticuloSoat As New ArticuloSoat()
        ...
        vArticuloSoat.dt_update = New DataTable() : vArticuloSoat.dt_delete = New DataTable

        cdna_insert = "" : cdna_update = "" : cdna_delete = "" : indcdor = 0 : bndra = True

        clmna_inicio = 4 : clmna_fin = 6
        cdna_insert = "INSERT INTO M_ARTICULOS_SOAT(Codigo_ARTICULO,Descripcion,CODIGO_capitulo) VALUES("
        cdna_update = "UPDATE M_ARTICULOS_SOAT SET Codigo_ARTICULO='_valor1',Descripcion='_valor2',CODIGO_SECCION='_valor3' WHERE Codigo_ARTICULO='_condicion'"
        cdna_delete = "DELETE M_ARTICULOS_SOAT WHERE Codigo_ARTICULO='_condicion'"
        ...
        For i = 0 To dgvprocedimiento.RowCount - 1
            ...
            ElseIf dgvprocedimiento.Rows(i).Cells("Edición").Value = True Then
                ...
                Else
                    ...
                    If vArticuloSoat.dt_update.Columns.Count <= 0 Then
                        vArticuloSoat.dt_update.Columns.Add("Consulta")
                    End If
                    vArticuloSoat.dt_update.Rows.Add()
                    vArticuloSoat.dt_update.Rows(vArticuloSoat.dt_update.Rows.Count - 1).Item(0) = cdna
                End If
            ElseIf dgvprocedimiento.Rows(i).Cells("Eliminación").Value = True Then
                ...
                If vArticuloSoat.dt_delete.Columns.Count <= 0 Then
                    vArticuloSoat.dt_delete.Columns.Add("Consulta")
                End If
                vArticuloSoat.dt_delete.Rows.Add()
                vArticuloSoat.dt_delete.Rows(vArticuloSoat.dt_delete.Rows.Count - 1).Item(0) = cdna_delete
            End If
        Next

        vArticuloSoat.indcdor = indcdor
        vArticuloSoat.cdna_insert = cdna_insert

        Return vArticuloSoat
    End Function
    ...
    Private Sub dgvprocedimiento_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvprocedimiento.CellDoubleClick
        If btguardar.Enabled = True Then
            If e.ColumnIndex = 6 Or e.ColumnIndex = 7 Or e.ColumnIndex = 8 Or e.ColumnIndex = 9 Then
                General.buscarElemento(Consultas.BUSQUEDA_ARTICULO_SOAT, Nothing, AddressOf cargar, TitulosForm.BUSQUEDA_SGCUPS, True)
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        enlce_dta.Filter = "[Código] LIKE '%" + TextBox1.Text.Trim() + "%' or [Descripción] LIKE '%" + TextBox1.Text.Trim() + "%' OR [Capitulo] LIKE '%" + TextBox1.Text.Trim() + "%' or [Descripción capitulo] LIKE '%" + TextBox1.Text.Trim() + "%' "
    End Sub
    ...
    Private Sub Form_ARTICULO_SOAT_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        llenar_dgv()
    End Sub
    ...
    Public Sub llenar_dgv()
        ...
        If dt.Rows.Count > 0 Then
            ...
            dgvprocedimiento.Columns(i - 3).AutoSizeMode = DataGridViewAutoSizeColumnMode.None
            ...
            dgvprocedimiento.Columns("Descripción").Width = "400"
            dgvprocedimiento.Columns(4).Width = "80"
            dgvprocedimiento.Columns(7).Width = "400"
