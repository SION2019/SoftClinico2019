Public Class Form_CATEGORIA_CUPS
    Dim enlce_dta As BindingSource = New BindingSource
    Dim i, c As Integer
    Dim cups As New CATCUPS_D
    ...
    Private Sub dgvprocedimiento_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvprocedimiento.CellDoubleClick
        If btguardar.Enabled = True Then
            If e.ColumnIndex = 6 Or e.ColumnIndex = 7 Then
                General.buscarItem(Consultas.CARGAR_SUBGRUPO_CUPS, Nothing,
                                       AddressOf cargarCups,
                                       TitulosForm.BUSQUEDA_SUBGCUPS,
                                       False)
        ...
        End If
    End Sub
    Public Sub cargarCups(pFila As DataRow)
        dgvprocedimiento.Rows(dgvprocedimiento.CurrentCell.RowIndex).Cells(6).Value = pFila.Item(0)
        dgvprocedimiento.Rows(dgvprocedimiento.CurrentCell.RowIndex).Cells(7).Value = pFila.Item(1)
    End Sub
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        TextBox1.Text = Funciones.validarComillaSimple(TextBox1.Text)
        enlce_dta.Filter = "[Código] LIKE '%" + TextBox1.Text.Trim() + "%' or [Descripción] LIKE '%" + TextBox1.Text.Trim() + "%' or [Subgrupo] LIKE '%" + TextBox1.Text.Trim() + "%' or [Descripción Subgrupo] LIKE '%" + TextBox1.Text.Trim() + "%'"
    End Sub
    ...
    Public Sub llenar_dgv()
        ...
        General.llenarTabla(Consultas.CARGAR_CATCUPS, params, dt)
