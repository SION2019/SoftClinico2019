Public Class NebulizacionRR
    ...
    Public Overrides Sub cargarNebulizacion()
        ...
        General.llenarTabla(ConsultasHC.NEBULIZACION_CARGAR_RR, params, tablaNebulacion)
    End Sub
    Public Overrides Sub getFechaEgreso()
        Dim params As New List(Of String)
        params.Add(nRegistro)
        Dim fila As DataRow
        fila = General.cargarItem(ConsultasHC.OXIGENO_FECHA_EGRESO_RR, params)
    ...
    Public Overrides Sub imprimirNebulizacion(ByVal lista As ListBox)
        ...
        If lista.SelectedIndex = 0 Then
            nombreArchivo = ConstantesHC.NOMBRE_PDF_NEBULIZACION_FISIO_RR & ConstantesHC.NOMBRE_PDF_SEPARADOR & nRegistro & ConstantesHC.EXTENSION_ARCHIVO_PDF
            ruta = Path.GetTempPath() & nombreArchivo
            ftp_reportes.llamarArchivo(ruta, nombreArchivo, nRegistro, ConstantesHC.NOMBRE_PDF_NEBULIZACION_FISIO_RR)
        Else
            nombreArchivo = ConstantesHC.NOMBRE_PDF_NEBULIZACION_FISIO_RR & ConstantesHC.NOMBRE_PDF_SEPARADOR & codigoNombre & ConstantesHC.EXTENSION_ARCHIVO_PDF
            ruta = Path.GetTempPath() & nombreArchivo
            formula = "{VISTA_FISIOTERAPIA_NEBULIZACION.Codigo_Orden} = " & codigoOrden & " AND {VISTA_FISIOTERAPIA_NEBULIZACION.Modulo}=" & modulo
            Dim reporte As New ftp_reportes
            reporte.crearYGuardarReporte(ConstantesHC.NOMBRE_PDF_NEBULIZACION_FISIO_RR,
                                         nRegistro, New rptFisioterapiaNebulizacion,
                                         codigoNombre,
                                         formula,
                                         ConstantesHC.NOMBRE_PDF_NEBULIZACION_FISIO_RR,
