Public Class MetaEmpleado
    ...
    Private Sub calcularMetaAlcanzada()
        For i = 0 To dtConsolidado.Rows.Count - 1
            cargarMetaDetalle(dtConsolidado.Rows(i).Item("Id_Empleado"))
            Dim dt As New DataTable
            dt = calcularDimensiones(dtConsolidado.Rows(i).Item("totalAportes"))
        ...
        Next
    End Sub
    Public Sub cargarMetaDetalle(idEmpleado As Integer)
        Dim params As New List(Of String)
        params.Add(SesionActual.codigoEP)
    ...
    End Sub
    Public Function calcularDimensiones(aporteActual As Double)
        ...
        If dtMetas.Rows.Count > 0 Then
            Dim distancia As Integer
            If dtMetas.Rows.Count > 1 Then
                distancia = 317 / (dtMetas.Rows.Count - 1)
            End If

            Dim localizacionSostenimiento As Integer
            Dim superada As Boolean
            If aporteActual >= dtMetas.Rows(0).Item("Valor") Then
                superada = True
            Else
                superada = False
            End If
            ...
            tablaLocalizacionMeta.Rows.Add(1, dtMetas.Rows(0).Item("Valor"), dtMetas.Rows(0).Item("Porcentaje"),
                                               localizacionSostenimiento, superada)
            For i = 1 To dtMetas.Rows.Count - 1
                localizacionSostenimiento = localizacionSostenimiento - distancia
                If aporteActual >= dtMetas.Rows(i).Item("Valor") Then
                    superada = True
                Else
                    superada = False
                End If
                tablaLocalizacionMeta.Rows.Add(i + 1, dtMetas.Rows(i).Item("Valor"), dtMetas.Rows(i).Item("Porcentaje"),
                localizacionSostenimiento, superada)
            Next
        End If
